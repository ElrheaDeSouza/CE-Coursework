...
public class BinarySegmentAdder {
    public static void main(String[] args) {
        ...
        // 4: Divide the binary data into segments
        ...
        // 5: Perform binary addition with carry discard rule
        ...
        // 6: Perform one's complement
        String onesComplement = onesComplement(result);

        // 7: Generate the final codeword (data + one's complement checksum)
    ...
    public static String addBinaryWithCarryDiscard(String bin1, String bin2) {
        ...
        if (sumBinary.length() > bin1.length()) {
            ...
            sumBinary = String.format("%" + bin1.length() + "s", Integer.toBinaryString(adjustedSum)).replace(' ', '0');
        } else {
            sumBinary = String.format("%" + bin1.length() + "s", sumBinary).replace(' ', '0');
        }
